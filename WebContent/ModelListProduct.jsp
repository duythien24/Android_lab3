...
<body>
	...
	<div class="col-sm-8 ">
		<c:forEach items="${sp}" var="sp">
			<div class="a">
				<form name="modelDetail" action="cart" method="get" class="form">
					Name: ${sp.name}<br /> <img src="${sp.image}" class="hinh">
					<br /> Price: ${sp.price}<br /> Quantity: ${sp.quantity}<br /> <input
						type="text" size="2" value="1" name="quantity"><br> <input
						type="hidden" value="${sp.productID}" name="id"> <input
						type="hidden" value="${sp.name}" name="name"> <input
						type="hidden" value="${sp.price}" name="price"> <input
